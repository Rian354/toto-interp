...
\begin{abstract}
Time-series foundation models are increasingly used for forecasting, monitoring, and operational decision support, but their internal representations remain poorly understood. We study \model{}, a multivariate forecasting foundation model, and ask whether its residual-stream states linearly encode observability-native concepts such as metric type, domain, sparsity, burstiness, shift risk, and multivariate coordination. We draft an end-to-end interpretability study around a reproducible research pipeline that traces patch-token activations, derives structural and dynamic labels from \boom{}, fits linear probes with raw-feature and shuffled-label controls, analyzes localization and geometry across layers and token views, and tests causal relevance through activation ablation and steering during forecasting. We further define zero-shot transfer tests on \fev{} and \lsf{} to evaluate whether \boom{}-trained dynamic probes capture benchmark-agnostic temporal regimes. This draft is grounded in the implemented codebase and the current research plan; numerical results, figure panels, and exact experimental counts remain marked as placeholders pending full-scale runs.
\end{abstract}
...
Our working thesis is that a time-series foundation model can contain internal representations of operational concepts that are both linearly accessible and behaviorally relevant. We decompose that thesis into three testable claims:
\begin{enumerate}[leftmargin=18pt]
\item structural and regime-level concepts should be decodable from residual activations above simple raw-patch baselines;
\item those concepts should localize non-uniformly across layers, token positions, and pooling views; and
\item intervening on selected concept directions should alter forecast behavior in conceptually coherent ways.
\end{enumerate}

The paper also stays explicit about scope. The current study is centered on a frozen pretrained \model{} checkpoint and linear probes. The accompanying research plan extends this baseline with randomized-weight controls, method selection between linear probes and an FNO-style supervised baseline, and optional checkpoint-loading for future fine-tuning studies. We treat those additions as follow-on controls rather than current claims.
...
\item It studies two concept families for operational time series: structural observability concepts and dynamic operating-regime concepts.
\item It organizes a NeurIPS-ready empirical narrative around the implemented workflow, including placeholders for structural decoding, dynamic localization, geometry, intervention, and transfer results.
\item It incorporates a principled roadmap for the next control experiments: randomized-weight baselines, dynamic method selection, and checkpoint-source metadata.
\end{enumerate}
...
This section keeps the result narrative in place while leaving empirical values to be filled from the final run artifacts.

\subsection{Structural observability concepts are linearly decodable}
...
The interpretation to verify is straightforward: if activation-based probes consistently beat raw-patch statistics, then \model{} is not only tracking local values but also organizing telemetry semantics in its hidden states.
...
The repo already highlights a testable architectural claim: layer 11, the space-wise mixing stage, may be disproportionately useful for coordination and cardinality. The final write-up should compare layer 11 to neighboring layers and report whether the gain is consistent across splits and label types.
...
The transfer analysis tests whether regime probes learned on observability data remain informative on non-observability forecasting tasks. Even partial transfer for regime-like variables such as sparsity or burstiness would support the view that the learned directions are not purely \boom{}-specific artifacts.
...
\section{Planned Controls and Extensions}
\label{sec:plan}

The active research plan extends the current frozen-pretrained linear-probe study in three focused ways.

\subsection{Randomized-weight controls}

The most important near-term control is to distinguish representational structure learned during pretraining from structure that arises from architecture alone. The planned loader extension therefore adds explicit model-source metadata for \texttt{pretrained}, \texttt{random\_init}, and \texttt{checkpoint} weight modes. The minimum viable control is full-model reinitialization after loading the public architecture.

\subsection{Method selection beyond linear probes}

The paper's current claims are intentionally probe-centered because interpretability depends on linear accessibility. The roadmap nevertheless adds runtime method selection between the current linear probe backend and an FNO-style supervised baseline trained on raw windows. This should be presented as a supervised comparison point rather than an interpretability artifact: localization and intervention claims remain tied to activation-backed linear probes unless a separate causal story is justified.

\subsection{Checkpoint-source and training-mode metadata}

The plan also formalizes distinctions that the paper should eventually report explicitly:
\begin{itemize}[leftmargin=18pt]
